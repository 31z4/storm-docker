...
* `1.0.3`, `1.0` [(1.0.3/Dockerfile)](https://github.com/31z4/storm-docker/blob/master/1.0.3/Dockerfile)
* `1.1.0`, `1.1`, `latest` [(1.1.0/Dockerfile)](https://github.com/31z4/storm-docker/blob/master/1.1.0/Dockerfile)
...
Assuming you have `topology.jar` in the current directory.
	...
	$ docker run -it -v $(pwd)/topology.jar:/topology.jar 31z4/storm:1.1 storm jar /topology.jar org.apache.storm.starter.ExclamationTopology
...
2.	The Nimbus daemon has to be connected with the Zookeeper. It's also a "fail fast" system.
		...
		$ docker run -d --restart always --name some-nimbus --link some-zookeeper:zookeeper 31z4/storm:1.1 storm nimbus

3.	Finally start a single Supervisor node. It will talk to the Nimbus and Zookeeper.

		$ docker run -d --restart always --name supervisor --link some-zookeeper:zookeeper --link some-nimbus:nimbus 31z4/storm:1.1 storm supervisor

4.	Now you can submit a topology to our cluster.

		$ docker run --link some-nimbus:nimbus -it --rm -v $(pwd)/topology.jar:/topology.jar 31z4/storm:1.1 storm jar /topology.jar org.apache.storm.starter.WordCountTopology topology

5.	Optionally, you can start the Storm UI.

		$ docker run -d -p 8080:8080 --restart always --name ui --link some-nimbus:nimbus 31z4/storm:1.1 storm ui
...
1.	Using command line arguments.
		...
		$ docker run -d --restart always --name nimbus 31z4/storm:1.1 storm nimbus -c storm.zookeeper.servers='["zookeeper"]'

2.	Assuming you have `storm.yaml` in the current directory you can mount it as a volume.

		$ docker run -it -v $(pwd)/storm.yaml:/conf/storm.yaml 31z4/storm:1.1 storm nimbus
...
No data are persisted by default. For convenience there are `/data` and `/logs` directories in the image owned by `storm` user. Use them accordingly to persist data and logs using volumes.
	...
	$ docker run -it -v /logs -v /data 31z4/storm:1.1 storm nimbus
